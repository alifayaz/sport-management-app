  ...
  return (
    <div>
      ...
      >
        ...
        <div
          className={twMerge(
            'flex ltr:flex-row-reverse items-center gap-2 bg-gray-100 border-gray-300 hover:border-primary transition-colors duration-200 rounded-2xl min-h-11 w-fit border focus-within:border-primary',
          ...
          <div className='flex justify-center'>
            <Select
              ...
              onSelectionChange={key => {
                const selectedItem = data.find((item:any) => item?.value === key);
                if (selectedItem) {
                  onChangeCodeValue?.(selectedItem?.label);
                }
              }}
              className='flex flex-col gap-1 relative'
            >
              <Button className='flex items-center justify-between min-w-60 cursor-default rounded-2xl bg-white transition py-2 px-4 text-base text-left leading-normal ring-1 ring-black/5 text-gray-700 focus:outline-hidden focus-visible:outline-2 outline-black outline-offset-3 focus-visible:ring-black/25'>
                {value ? value : placeholder}
              </Button>
              <Popover
                placement='bottom left'
                className='!max-h-80 w-fit flex flex-col max-w-60 rounded-2xl bg-white text-base shadow:[0px_0px_4px_0px_rgba(0,0,0,0.25)]
